...
export interface CatalogProduct {
  ...
  name: string;
  price: number;
  imageUrl: string; // Google Drive public link (uc?export=view&id=...)
...
export class CatalogService {
  ...
  add(product: { name: string; price: number; imageUrl: string; category: string; description: string }): void {
    ...
    list.push({ id: newId, name: product.name, price: product.price, imageUrl: product.imageUrl, category: product.category, description: product.description });
    this.products$.next(list);
  }
